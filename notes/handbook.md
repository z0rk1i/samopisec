...
- **Sideload-сборки (Phase 6)** — EAS НЕ используем (делает `prebuild --clean` и стирает виджет-таргеты). Только локально:
  - Android release APK: `source scripts/env.sh && clojure -M -m shadow.cljs.devtools.cli release app` → `cd android && ./gradlew assembleRelease` → `android/app/build/outputs/apk/release/app-release.apk` (~23M, подписан debug-ключом — ставится на любые устройства). Сборка arm64-only + R8 (ADR-0019); x86/x86_64 при необходимости: `./gradlew assembleRelease -PreactNativeArchitectures=x86_64`
