...
## ADR-0018 — Android-виджет: добавление работает, а редактирование/удаление не отображалось (инкрементальный reapply лаунчера vs динамический addView)

**Дата:** 2026-08-17
**Статус:** принято, проверено на эмуляторе Android 16

### Контекст
После ADR-0017 (JS→native мост `:widget/refresh` восстановлен) цепочка
`:config/commit → :storage/save-config → :widget/refresh → WidgetBridge →
updateAppWidget` полностью работает: logcat показывает `onUpdate`, `updateAppWidgetIds`
возвращает успех, `config.json` на диске правильный. Но пользователь сообщил:
добавление кнопки обновляет виджет, а редактирование и удаление — нет.

### Причина
Лаунчер (AppWidgetHostView) применяет RemoteViews-апдейт с **тем же layout id**
как инкрементальный **reapply к существующему дереву** (AppWidgetHostView.updateAppWidget:
`if (layoutId == mRemoteViews.getLayoutId()) mRemoteViews.reapply(...)`).

Старый `buildViews` строил кнопки динамически: `root.addView(slotId, buttonView)`.
При reapply новые кнопки **добавлялись поверх старых**, а старые не удалялись —
видимое содержимое не менялось. Поэтому:
- **добавление** кнопки «работало» — слот получал первого ребёнка;
- **редактирование/удаление** невидимы — дерево уже содержало старые кнопки.

Перерисовка происходила только при смене layout id (re-inflate всего дерева) —
отсюда симптом «удалить виджет и добавить заново».

### Решение
1. **6 слотов зашиты статически** в новый layout `widget_layout_v2.xml`
   (id `widget_button_root_1..6` + `widget_label_1..6`, все `visibility=gone`).
2. `buildViews` использует **только replace-операции**: `setViewVisibility`,
   `setTextViewText`, `setTextViewTextSize`, `setInt(setBackgroundColor)`,
   `setContentDescription`, `setOnClickPendingIntent`. Никаких `addView`.
3. **Новый layout id** (`widget_layout_v2`, а не `widget_layout`): старые
   сохранённые системой RemoteViews используют старую структуру. Первый апдейт
   с новым кодом меняет layout id → лаунчер принудительно re-inflate'ит виджет
   (смена layout рендерится всегда), дальше все апдейты идут по v2 → reapply
   работает на новом дереве.
4. Старые `widget_layout.xml` и `widget_button.xml` **оставлены** — старые
   сохранённые RemoteViews (до первого нового апдейта) ссылаются на них при
   re-inflate (перезагрузка лаунчера/устройства).

### Проверка (эмулятор Android 16, release APK, реальный UI-поток)
- **Редактирование цвета (reapply-путь)**: Кофе зелёный → синий (#1e88e5) →
  `onUpdate` → виджет перерисован (пиксель-проверка: 33160 px #1E88E5,
  старые синий/зелёный исчезли). Повторный эдит синий → красный (#e53935) —
  тот же v2 layout → **виджет обновился live** (33163 px #E53935): reapply работает.
- **Удаление**: ✕ → «Удалить кнопку?» → УДАЛИТЬ → `config.json={"buttons":[]}`
  → `onUpdate` → виджет показал пустое состояние (текст #9E9E9E на тёмном фоне).
- **Восстановление**: [Чай,Кофе] → виджет показывает обе кнопки
  (33637 px #1976D2 + 33173 px #43A047 — те же сигнатуры, что в исходном состоянии).
- `npm run lint` 0/0; CLJS: 66 файлов, 0 warnings; 38 тестов / 161 assertion,
  0 failures; swift typecheck чистый; gradle assembleRelease успешен.
...
## ADR-0023 — Производные перенесены в offline Grafana («Производная 1»/«Производная 2»)

**Дата:** 2026-08-23
**Статус:** принято

### Контекст
После перехода вкладки «Графики» на offline-Grafana (ADR-0022) производные кривые (быв. «Скорость»/«Ускорение») стали недостижимы: старый экран `ui/charts.cljs` не рендерится, а в Grafana-дашборде их нет.

### Решение
1. В `assets/grafana-offline/index.html` добавлены панели **«Производная 1»** (нажатий/час) и **«Производная 2»** (Δ нажатий/час²) per-button цветом кнопки; `offline_html.cljs` перегенерирован из index.html (живая копия для WebView — CLJS-строка).
2. Математика — точный порт `app.math`: авто-бин по длине диапазона (<3д→1ч, <14д→6ч, иначе 24ч), число бинов округляется вверх (хвост сохраняется), rate = count/бин-часы, accel = центральная разность MA(5)-сглаженного rate / (2·dt). X — общий глобальный span всех точек.
3. Мёртвый нативный экран графиков удалён: `src/app/ui/charts.cljs`, `app/timeline.cljs` (+тест), ключи i18n `:charts/*`. Слой re-frame-состояния графиков (`events/chart.cljs`, селекторы серий) оставлен — `:today/counts` нужен экрану кнопок, серийная математика протестирована и может пригодиться.
4. Панель P2 рисует пунктирную нулевую линию при отрицательных значениях.

### Проверка
Node smoke-тест порта (17 проверок против эталонных значений формул) — pass; headless-прогон drawDerivative со стабом 2D-контекста (сетка/полилинии/нулевая линия/пустые данные) — pass; clj-kondo 0/0; 43 теста / 174 assertion pass; `shadow compile app` 0 warnings. Живой WebView — проверить на устройстве.

## ADR-0024 — Spill-файл тапов виджетов: однописательный основной CSV

**Дата:** 2026-08-23
**Статус:** принято

### Контекст
Виджеты дописывали тапы прямо в datapoints.csv, минуя очередь записи приложения:
атомарная замена файла (undo/компакция) могла съесть тап, записанный между чтением
и moveSync; нативный header-fix делал неатомарную полную перезапись. Окно гонки
app↔виджет — единственный источник потери данных.

### Решение
1. Виджеты (TapWidgetProvider.kt, widgets.swift) пишут тапы ТОЛЬКО в
   `datapoints-spill.csv` (тот же header id,button_id,ts; append-only; создают
   файл с header при первом тапе). Старый header-fix перезаписью удалён.
2. Приложение — единственный писатель основного файла. Внутри write-queue перед
   каждым чтением (`read-datapoints`, undo) выполняется дренаж: строки spill
   append'ятся в main, затем spill удаляется.
3. Порядок «append → delete spill» устойчив к крашу: недобитое останется в spill,
   возможный дубль гасится дедупликацией по :id при чтении
   (contract/dedupe-by-id, первый вхождение побеждает).
4. Undo после компакции: если основной файл пуст/отсутствует, последняя строка
   снимается из архива (delete-last-from-archive!).

### Проверка
contract/dedupe-by-id покрыт тестами; kondo 0/0; swift typecheck чистый;
эмулятор: broadcast ACTION_TAP пишет в spill, запуск приложения дренажует в main
(см. notes/tasks.md сессию 2026-08-23).

### Последствия
+ Основной файл однописательный — класс гонок «тап потерян при replace» исчез.
− Тап виджета виден приложению только после дренажа (первое чтение после
  foreground/запуска) — как и раньше до перезагрузки db.
